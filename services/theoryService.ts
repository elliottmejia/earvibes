...
import { Chord, LevelType, Progression } from '../types';
...
export const generateProgression = (levelId: number, type: LevelType): Progression => {
  ...
  switch (type) {
    ...
    case LevelType.DORIAN:
      ...
      map = A_MINOR_TRIADS;
      // In Dorian, the IV is Major. ii is Minor (not diminished).
    ...
    case LevelType.CITY_POP:
      key = "C Major (Tatsuro)";
      map = C_CITY_POP;
      // We define the pool, but we might override generation logic below
  ...
  if (firstChordNotes) {
      result.push({ roman: firstChordStr, notes: firstChordNotes, displayName: `${firstChordStr} (${key})` });
  }
...
export const getChordNotes = (roman: string, levelType: LevelType): string[] => {
   if (levelType === LevelType.MINOR || levelType === LevelType.DORIAN) return A_MINOR_TRIADS[roman] || [];
   if (levelType === LevelType.CITY_POP) return C_CITY_POP[roman] || [];
   if (levelType === LevelType.OUDOU || levelType === LevelType.TRITONE_SUB) return C_TETRADS[roman] || [];
   if (levelType === LevelType.SECONDARY_DOMINANT && SECONDARY_DOMINANT_7THS[roman]) return SECONDARY_DOMINANT_7THS[roman] || [];
